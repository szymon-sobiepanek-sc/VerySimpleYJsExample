var crdt = (function (exports) {
  ...
  const readAuthMessage = (decoder, y, permissionDeniedHandler) => {
    switch (readVarUint(decoder)) {
      case messagePermissionDenied:
        permissionDeniedHandler(y, readVarString(decoder));
    }
  };
  ...
  */
  ...
  const setupWS = provider => {
    if (provider.shouldConnect && provider.ws === null) {
      ...
      websocket.onopen = () => {
        ...
        websocket.send(toUint8Array(encoder)); // broadcast local awareness state
  ...
  class WebsocketProvider extends Observable {
    ...
    connectBc() {
      ...
      this.mux(() => {
        ...
        publish(this.bcChannel, toUint8Array(encoderSync)); // broadcast local state

        const encoderState = createEncoder();
        writeVarUint(encoderState, messageSync);
        writeSyncStep2(encoderState, this.doc);
        publish(this.bcChannel, toUint8Array(encoderState)); // write queryAwareness

        const encoderAwarenessQuery = createEncoder();
        writeVarUint(encoderAwarenessQuery, messageQueryAwareness);
        publish(this.bcChannel, toUint8Array(encoderAwarenessQuery)); // broadcast local awareness state
  ...
  var bundle = function bundle() {
    ...
    var ydoc = new Doc();
    var provider = new WebsocketProvider("ws://localhost:4444", "something-here", ydoc);
